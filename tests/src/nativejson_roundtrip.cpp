...
#endif
{
	...
	using policy_t =
	  daw::json::DefaultParsePolicy::template SetPolicyOptions<daw::json::IEEE754Precise::yes>;
	std::cout << "C++ DAW JSON Link\n";
	auto const canada_obj =
	  daw::json::from_json<daw::geojson::Polygon, policy_t>( canada_data, "features[0].geometry" );
	auto const twitter_obj =
	  daw::json::from_json<daw::twitter::twitter_object_t, policy_t>( twitter_data );
	auto const citm_obj = daw::json::from_json<daw::citm::citm_object_t, policy_t>( citm_data );
	...
	auto const canada_obj2 = daw::json::from_json<daw::geojson::Polygon, policy_t>( canada_out );
	auto const twitter_obj2 =
	  daw::json::from_json<daw::twitter::twitter_object_t, policy_t>( twitter_out );
	auto const citm_obj2 = daw::json::from_json<daw::citm::citm_object_t, policy_t>( citm_out );
	...
	std::cout << "canada roundtrip: " << ( canada_out == canada_out2 ? "good" : "bad" ) << '\n';

	std::cout << "twitter roundtrip: " << ( twitter_out == twitter_out2 ? "good" : "bad" ) << '\n';
	...
	if( twitter_out != twitter_out2 ) {
		...
		auto pos = std::mismatch( std::data( twitter_out ),
