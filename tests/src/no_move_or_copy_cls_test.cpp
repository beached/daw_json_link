...
#endif
{
	...
	using policy_t =
	  daw::json::BasicParsePolicy<daw::json::parse_options( daw::json::ExecModeTypes::runtime )>;

	daw::expecting( daw::json::from_json<A, policy_t>( json_data ).member == 1234 );

	std::string const json_data2 = R"({ "a": { "some_num": 1234 } } )";
	daw::expecting( daw::json::from_json<B, policy_t>( json_data2 ).a.member == 1234 );
}
#ifdef DAW_USE_EXCEPTIONS
