...
struct Foo {
	...
	std::tuple<double, std::string, int, std::variant<Bar, Umm>> o{ 1.1, "2", 1, Umm{} };
};
...
struct FooBoo {
	...
	std::tuple<int, double> m{ 99, 98.8 };
	std::variant<Bar, Umm> n{ Umm{} };
	std::tuple<double, std::string_view, int, std::variant<Bar, Umm>> o{ 1.1, "2", 1, Umm{} };
};
...
struct UniquePtrArrayCtor {
	...
	inline std::unique_ptr<T[]> operator( )( Iterator first, Iterator last, std::size_t sz ) const {
...
namespace daw::json {
	...
	struct json_data_contract<Foo> {
		...
		using type =
		  json_member_list<json_link<mem_a, int>,
