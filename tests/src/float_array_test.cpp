...
void test_func( ) {
	...
	// Normal
	{
		...
		using iterator_t = daw::json::json_array_iterator<json_class_no_name<Number>>;
	...
	{ // just ints
		...
		std::cout << "element count: " << count << '\n';
		using iterator_t = daw::json::json_array_iterator<json_number_no_name<float>>;
	...
	{ // just ints
		...
		auto const count = *daw::bench_n_test_mbs<DAW_NUM_RUNS>(
		  ...
		  json_sv.size( ),
		  []( auto &&sv ) noexcept {
			  auto const data = from_json_array<json_checked_number_no_name<float>>( sv );
			  daw::do_not_optimize( data );
			  return data.size( );
		  },
		  json_sv );
		...
		std::cout << "element count: " << count << '\n';
		using iterator_t = daw::json::json_array_iterator<json_checked_number_no_name<float>>;
	...
	std::cout << "**********************\nSSE42 Processing\n";
	{
		...
		auto const count = *daw::bench_n_test_mbs<DAW_NUM_RUNS>(
		  ...
		  json_sv.size( ),
		  []( auto &&sv ) noexcept {
			  auto const data = from_json_array<Number2,
			                                    std::vector<Number2>,
	...
	{ // just floats
		...
		auto const count = *daw::bench_n_test_mbs<DAW_NUM_RUNS>(
		  ...
		  json_sv.size( ),
		  []( auto &&sv ) noexcept {
			  auto const data = from_json_array<json_number_no_name<float>,
			                                    std::vector<float>,
			                                    SIMDNoCommentSkippingPolicyChecked<sse42_exec_tag>>( sv );
			  daw::do_not_optimize( data );
			  return data.size( );
