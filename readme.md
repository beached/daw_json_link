...
```
# Error Handling
There are two possible ways of handling errors.  The first, `abort( );` on an error in data.  Or the, second, throw a `daw::json::json_exception`.  json_exception has a member function `std::string_view reason( ) const`.  You can control which method is used by defining `DAW_JSON_USE_EXCEPTIONS`.  In addition, you can control if the checks are only done in debug mode or always by defining `DAW_JSON_CHECK_ALWAYS`.  In the future, the defaults will be to use the exception and always check.
