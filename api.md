...
template<JSONNAMETYPE Name, 
  typename T = double,
  LiteralAsStringOpt LiteralAsString = LiteralAsStringOpt::NotBeforeDblQuote,
  typename Constructor = daw::construct_a_t<T>,
  JsonRangeCheck RangeCheck = JsonRangeCheck::NotBeforeDblQuote,
  JsonNullable Nullable = JsonNullable::NotBeforeDblQuote>
struct json_number
``` 
...
- ```LiteralAsString``` allow specifying if the number is stored as a string.  Values are ```LiteralAsStringOpt::NotBeforeDblQuote```, ```LiteralAsStringOpt::Maybe```, and ```LiteralAsStringOpt::Always```.
- ```Constructor``` the default is almost always correct here but this will constuct your type. 
- ```RangeCheck``` ensures that the number isn't narrowed when stored as your type. Values are ```JsonRangeCheck::NotBeforeDblQuote```, and ```JsonRangeCheck::CheckForNarrowing```
...
template<JSONNAMETYPE Name, 
  ...
  JsonNullable EmptyStringNull = JsonNullable::NotBeforeDblQuote,
...
template<JSONNAMETYPE Name,
  ...
  typename Constructor = construct_from_iso8601_timestamp<JsonNullable::NotBeforeDblQuote>,
  JsonNullable Nullable = JsonNullable::NotBeforeDblQuote>
struct json_date
``` 
