...
#pragma GCC diagnostic pop
	...
	namespace detail {
		...
		struct impl : private ieee754_traits<Float>,
			...
			static constexpr int case_shorter_interval_left_endpoint_lower_threshold =
			  2;
			  ...
			  case_shorter_interval_right_endpoint_upper_threshold =
			    ...
			    log::floor_log2(
			      compute_power<count_factors<5>(
			                      ( carrier_uint( 1 ) << ( significand_bits + 1 ) ) +
			                      1 ) +
			                    1>( 10 ) /
			      3 );
			...
			compute_right_closed_directed( ieee754_bits<Float> const br ) noexcept {
				...
				// Compute zi and deltai
				// 10^kappa <= deltai < 10^(kappa + 1)
				auto const deltai = closer_boundary ? compute_delta( cache, beta - 2 )
			...
			compute_round_up_for_shorter_interval_case( cache_entry_type const &cache,
			                                            int beta_minus_1 ) noexcept {
				if constexpr( format == ieee754_format::binary32 ) {
					return ( carrier_uint( cache >> ( cache_bits - significand_bits - 2 -
					                                  beta_minus_1 ) ) +
					         1 ) /
					       2;
				} else {
					return ( ( cache.high( ) >>
					           ( carrier_bits - significand_bits - 2 - beta_minus_1 ) ) +
					         1 ) /
					       2;
				}
			}
