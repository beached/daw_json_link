...
#endif
#if defined( DAW_JSON_RYU_HAS_UINT128 )
...
namespace ryu::detail {
	...
	inline std::uint64_t const ( &DOUBLE_POW5_SPLIT2( ) noexcept )[13][2] {
	...
	// Unfortunately, the results are sometimes off by one. We use an
	// additional lookup table to store those cases and adjust the result.
	inline std::uint32_t const ( &POW5_OFFSETS( ) noexcept )[13] {
	...
	inline std::uint64_t const ( &DOUBLE_POW5_INV_SPLIT2( ) noexcept )[13][2] {
		static constexpr std::uint64_t arr[13][2] = {
	...
	inline std::uint32_t const ( &POW5_INV_OFFSETS( ) noexcept )[20] {
	...
	// Computes 5^i in the form required by Ryu, and stores it in the
	// given pointer.
	inline void double_computePow5( const std::uint32_t i,
...
#else // defined(DAW_JSON_RYU_HAS_UINT128)
...
#endif // defined(DAW_JSON_RYU_HAS_UINT128)

#endif // defined(DAW_JSON_RYU_OPTIMIZE_SIZE)
