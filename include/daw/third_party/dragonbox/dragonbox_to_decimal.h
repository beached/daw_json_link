...
namespace daw::jkj::dragonbox {
	DAW_JSON_INLINE_NS namespace DAW_JSON_VER {
		...
		struct default_ieee754_traits {
			...
			using type = T;
			static constexpr ieee754_format format = detail::physical_bits<T> == 32
		...
		struct ieee754_bits {
			...
			[[nodiscard]] inline constexpr carrier_uint
			...
			is_positive_infinity( ) const noexcept {
				return ieee754_traits<T>::is_positive_infinity( u );
			}

			[[nodiscard]] inline constexpr bool
			is_negative_infinity( ) const noexcept {
...
#pragma GCC diagnostic pop
  ...
  defined( _M_X64 )
			...
			namespace div {
				...
				[[nodiscard]] inline constexpr UInt modular_inverse(
				...
				[[nodiscard]] inline constexpr bool
				divisible_by_power_of_5( UInt x, unsigned exp ) noexcept {
					auto const &table =
				...
				[[nodiscard]] inline constexpr UInt divide_by_pow10( UInt n ) noexcept {
					...
					// Ensure no overflow
					static_assert( max_pow2 +
					                 ( log::floor_log2_pow10( max_pow5 ) - max_pow5 ) <
					               value_bits<UInt> );
			...
			struct compressed_cache_detail {
				...
				static constexpr std::size_t compressed_table_size =
				  ( cache_holder<ieee754_format::binary64>::max_k -
				    cache_holder<ieee754_format::binary64>::min_k +
				    compression_ratio ) /
				  compression_ratio;
				...
				static constexpr cache_holder_t cache = [] {
					...
					for( std::size_t i = 0; i < compressed_table_size; ++i ) {
						res.table[i] =
						  cache_holder<ieee754_format::binary64>::cache[i *
						                                                compression_ratio];
					}
					return res;
				}( );
			...
			namespace policy_impl {
				...
				namespace correct_rounding {
					...
					struct away_from_zero : base {
						...
						break_rounding_tie( Fp & /*fp*/ ) noexcept {}
					};
						  ...
						  get_cache( int k ) noexcept {
							...
							if constexpr( format == ieee754_format::binary64 ) {
								...
								if( offset == 0 ) {
									...
									auto middle_low =
									  wuint::umul128( base_cache.low( ) - ( kb < 0 ? 1 : 0 ),
									                  pow5 );

									recovered_cache += middle_low.high( );

									auto high_to_middle = recovered_cache.high( )
									                      << ( 64 - alpha );
									...
									auto error =
									  ( compressed_cache_detail::errors[error_idx] >>
									    ( ( k - cache_holder<format>::min_k ) % 16 ) * 2 ) &
									  0x3;

									// Add the error back
									assert( recovered_cache.low( ) + error >=
									        recovered_cache.low( ) );
		...
		namespace detail {
			...
			struct impl :
			  ...
			  private ieee754_format_info<ieee754_traits<Float>::format> {
				...
				static constexpr int case_fc_pm_half_upper_threshold =
				  log::floor_log2_pow10( kappa + 1 );

				static constexpr int case_fc_lower_threshold =
				  -kappa - 1 - log::floor_log5_pow2( kappa + 1 );
				static constexpr int case_fc_upper_threshold =
				  log::floor_log2_pow10( kappa + 1 );
				...
				compute_nearest( ieee754_bits<Float> const br ) noexcept {
					...
					auto const interval_type =
					  IntervalTypeProvider::interval_type_normal( br );
				...
				compute_right_closed_directed( ieee754_bits<Float> const br ) noexcept {
					...
					// Compute k and beta
					int const minus_k =
					  log::floor_log10_pow2( exponent - ( closer_boundary ? 1 : 0 ) ) -
					  kappa;
					auto const cache =
					  CachePolicy::template get_cache<format>( -minus_k );
					int const beta = exponent + log::floor_log2_pow10( -minus_k ) + 1;
					...
					if( r > deltai ) {
						...
						// Compare the fractional parts
						if( closer_boundary ) {
							if( not compute_mul_parity( ( significand * 2 ) - 1,
							                            cache,
							                            beta - 1 ) ) {
								return small_divisor_case_label( );
							}
				...
				[[nodiscard]] static inline std::uint32_t constexpr compute_delta(
				  ...
				  int beta_minus_1 ) noexcept {
					if constexpr( format == ieee754_format::binary32 ) {
						...
						return std::uint32_t( cache.high( ) >>
						                      ( carrier_bits - 1 - beta_minus_1 ) );
					}
				}
				...
				compute_round_up_for_shorter_interval_case(
				  ...
				  int beta_minus_1 ) noexcept {
					if constexpr( format == ieee754_format::binary32 ) {
						...
						return ( ( cache.high( ) >> ( carrier_bits - significand_bits - 2 -
						                              beta_minus_1 ) ) +
						         1 ) /
						       2;
					}
				...
				[[nodiscard]] static inline constexpr bool
				is_product_integer( carrier_uint two_f,
				                    int exponent,
			...
			namespace policy_impl {
				...
				struct base_default_pair {
					...
					[[nodiscard]] static constexpr FoundPolicyInfo
				...
				make_policy_holder( BaseDefaultPairList, Policies... policies ) {
					static_assert( check_policy_list_validity( BaseDefaultPairList{ },
					                                           Policies{ }... ),
		  ...
		  Policies... policies ) {
			...
			using policy_holder = decltype( make_policy_holder(
			  base_default_pair_list<
			    ...
			    base_default_pair<input_validation::base,
			                      input_validation::assert_finite>>{ },
			  policies... ) );
