...
namespace daw::jkj::dragonbox {
	DAW_JSON_INLINE_NS namespace DAW_JSON_VER {
		...
		struct default_ieee754_traits {
			static_assert( detail::physical_bits<T> == 32 or detail::physical_bits<T> == 64 );
			...
			static inline constexpr unsigned int extract_exponent_bits( carrier_uint u ) noexcept {
			...
			static inline constexpr carrier_uint extract_significand_bits( carrier_uint u ) noexcept {
				constexpr int significand_bits = ieee754_format_info<format>::significand_bits;
			...
			static constexpr int exponent_bias =
			  1 - ( 1 << ( carrier_bits - ieee754_format_info<format>::significand_bits - 2 ) );
			...
			static inline constexpr bool is_positive_infinity( carrier_uint u ) noexcept {
				constexpr int significand_bits = ieee754_format_info<format>::significand_bits;
				constexpr int exponent_bits = ieee754_format_info<format>::exponent_bits;
			...
			static inline constexpr bool is_negative_infinity( carrier_uint u ) noexcept {
				constexpr int significand_bits = ieee754_format_info<format>::significand_bits;
				constexpr int exponent_bits = ieee754_format_info<format>::exponent_bits;
		...
		struct ieee754_traits : default_ieee754_traits<T> {
			...
			static_assert( std::numeric_limits<T>::is_iec559 and std::numeric_limits<T>::radix == 2 and
		...
		struct ieee754_bits {
			...
			[[nodiscard]] inline constexpr T to_float( ) const noexcept {
				return ieee754_traits<T>::carrier_to_float( u );
			}

			[[nodiscard]] inline constexpr carrier_uint extract_significand_bits( ) const noexcept {
				return ieee754_traits<T>::extract_significand_bits( u );
			}

			[[nodiscard]] inline constexpr unsigned extract_exponent_bits( ) const noexcept {
				return ieee754_traits<T>::extract_exponent_bits( u );
			}

			[[nodiscard]] inline constexpr carrier_uint binary_significand( ) const noexcept {
			...
			[[nodiscard]] inline constexpr int binary_exponent( ) const noexcept {
				using format_info = ieee754_format_info<ieee754_traits<T>::format>;
			...
			[[nodiscard]] inline constexpr bool is_finite( ) const noexcept {
				return ieee754_traits<T>::is_finite( u );
			}

			[[nodiscard]] inline constexpr bool is_nonzero( ) const noexcept {
				return ieee754_traits<T>::is_nonzero( u );
			}
			// Allows positive and negative zeros
			[[nodiscard]] inline constexpr bool is_subnormal( ) const noexcept {
				return ieee754_traits<T>::is_subnormal( u );
			}
			// Allows positive zero and positive NaN's, but not allow negative zero
			[[nodiscard]] inline constexpr bool is_positive( ) const noexcept {
				return ieee754_traits<T>::is_positive( u );
			}
			// Allows negative zero and negative NaN's, but not allow positive zero
			[[nodiscard]] inline constexpr bool is_negative( ) const noexcept {
				return ieee754_traits<T>::is_negative( u );
			}

			[[nodiscard]] inline constexpr bool is_positive_infinity( ) const noexcept {
				return ieee754_traits<T>::is_positive_infinity( u );
			}

			[[nodiscard]] inline constexpr bool is_negative_infinity( ) const noexcept {
				return ieee754_traits<T>::is_negative_infinity( u );
			}
			// Allows both plus and minus infinities
			[[nodiscard]] inline constexpr bool is_infinity( ) const noexcept {
				return ieee754_traits<T>::is_infinity( u );
			}
			[[nodiscard]] inline constexpr bool is_nan( ) const noexcept {
...
#if( defined( __GNUC__ ) or defined( __clang__ ) ) and defined( __SIZEOF_INT128__ ) and \
  defined( __x86_64__ )
#ifdef __GNUC__
...
#pragma GCC diagnostic ignored "-Wpedantic"
					...
					constexpr uint128( std::uint64_t high, std::uint64_t low ) noexcept
					  : internal_{ ( static_cast<unsigned __int128>( low ) ) |
...
#pragma GCC diagnostic pop
					...
					constexpr uint128 &operator+=( std::uint64_t n ) &noexcept {
#if defined( DAW_IS_CONSTANT_EVALUATED ) and defined( _MSC_VER ) and defined( _M_X64 )
				...
				// integers
				[[nodiscard]] JKJ_SAFEBUFFERS inline constexpr uint128 umul128( std::uint64_t x,
...
#pragma GCC diagnostic pop
#endif
#elif defined( _MSC_VER ) and defined( _M_X64 ) and defined( DAW_IS_CONSTANT_EVALUATED )
...
#pragma GCC diagnostic pop
#endif
#elif defined( DAW_IS_CONSTANT_EVALUATED ) and defined( _MSC_VER ) and defined( _M_X64 )
					if( DAW_IS_CONSTANT_EVALUATED( ) ) {
						constexpr auto mask = ( std::uint64_t( 1 ) << 32 ) - std::uint64_t( 1 );
				...
				[[nodiscard]] inline constexpr std::uint32_t umul96_upper32( std::uint32_t x,
				                                                             std::uint64_t y ) noexcept {
				...
				// and a 64-bit unsigned integer
				[[nodiscard]] inline constexpr std::uint64_t umul96_lower64( std::uint32_t x,
				                                                             std::uint64_t y ) noexcept {
					return x * y;
				}
			} // namespace wuint
			...
			[[nodiscard]] inline constexpr Int compute_power( Int a ) noexcept {
				static_assert( k >= 0 );
				Int p = 1;
				for( int i = 0; i < k; ++i ) {
					p *= a;
			...
			[[nodiscard]] inline constexpr int count_factors( UInt n ) noexcept {
				static_assert( a > 1 );
				int c = 0;
				while( n % a == 0 ) {
					n /= a;
			...
			namespace log {
				...
				floor_shift( std::uint32_t integer_part,
				...
				// Compute floor(e * c - s)
				template<std::uint32_t c_integer_part,
				...
				[[nodiscard]] inline constexpr int compute( int e ) noexcept {
					...
					return static_cast<int>( ( std::int32_t( e ) * c - s ) >> shift_amount );
				}
				...
				// Returns -1 when n = 0
				template<class UInt>
				[[nodiscard]] inline constexpr int floor_log2( UInt n ) noexcept {
					int count = -1;
					while( n != 0 ) {
						++count;
						n >>= 1;
				...
				[[nodiscard]] inline constexpr int floor_log10_pow2( int e ) noexcept {
					using namespace log;
					return compute<0, log10_2_fractional_digits, floor_log10_pow2_shift_amount, 1700>( e );
				}

				[[nodiscard]] inline constexpr int floor_log2_pow10( int e ) noexcept {
					using namespace log;
					return compute<3, log2_10_fractional_digits, floor_log2_pow10_shift_amount, 1233>( e );
				}

				[[nodiscard]] inline constexpr int floor_log5_pow2( int e ) noexcept {
					using namespace log;
					return compute<0, log5_2_fractional_digits, floor_log5_pow2_shift_amount, 1492>( e );
				}

				[[nodiscard]] inline constexpr int floor_log5_pow2_minus_log5_3( int e ) noexcept {
					using namespace log;
					return compute<0,
				...
				[[nodiscard]] inline constexpr int floor_log10_pow2_minus_log10_4_over_3( int e ) noexcept {
					using namespace log;
					return compute<0,
			...
			namespace div {
				...
				[[nodiscard]] inline constexpr UInt
				modular_inverse( int bit_width = static_cast<int>( value_bits<UInt> ) ) noexcept {
				...
				[[nodiscard]] inline constexpr bool divisible_by_power_of_5( UInt x,
				                                                             unsigned exp ) noexcept {
					auto const &table = table_holder<UInt, 5, static_cast<int>( table_size )>::table;
					assert( exp < static_cast<unsigned>( table.size ) );
					return ( x * table.mod_inv[exp] ) <= table.max_quotients[exp];
				}

				template<class UInt>
				[[nodiscard]] inline constexpr bool divisible_by_power_of_2( UInt x,
				                                                             unsigned exp ) noexcept {
					assert( exp >= 1 );
					assert( x != 0 );
				...
				[[nodiscard]] inline constexpr UInt divide_by_pow10( UInt n ) noexcept {
					static_assert( N >= 0 );
			...
			namespace policy_impl {
				...
				namespace sign {
					...
					struct ignore : base {
						...
						static inline constexpr void handle_sign( ieee754_bits<Float>, Fp & ) noexcept {}
					};
					...
					struct return_sign : base {
						...
						template<class Float, class Fp>
						static inline constexpr void handle_sign( ieee754_bits<Float> br, Fp &fp ) noexcept {
				...
				namespace trailing_zero {
					...
					struct remove : base {
						...
						static inline constexpr void on_trailing_zeros( Fp &fp ) noexcept {
							fp.exponent += impl<typename Fp::float_type>::remove_trailing_zeros( fp.significand );
						}
				...
				namespace rounding_mode {
					...
					namespace interval_type {
						struct symmetric_boundary {
							...
							bool is_closed;
							[[nodiscard]] inline constexpr bool include_left_endpoint( ) const noexcept {
								return is_closed;
							}

							[[nodiscard]] inline constexpr bool include_right_endpoint( ) const noexcept {
								return is_closed;
							}
						};
						...
						struct closed {
							static constexpr bool is_symmetric = true;
							[[nodiscard]] static inline constexpr bool include_left_endpoint( ) noexcept {
								return true;
							}
							[[nodiscard]] static inline constexpr bool include_right_endpoint( ) noexcept {
								return true;
							}
						};
						struct open {
					...
					struct nearest_to_even : base {
						...
						[[nodiscard]] static inline constexpr auto delegate( ieee754_bits<Float>,
						                                                     Func &&f ) noexcept {
							return f( nearest_to_even{ } );
						}

						template<class Float>
						[[nodiscard]] static inline constexpr interval_type::symmetric_boundary
					...
					struct nearest_toward_plus_infinity : base {
						...
						template<class Float>
						[[nodiscard]] static inline constexpr interval_type::asymmetric_boundary
						interval_type_normal( ieee754_bits<Float> br ) noexcept {
					...
					struct nearest_toward_zero : base {
						...
						template<class Float>
						[[nodiscard]] static inline constexpr interval_type::right_closed_left_open
					...
					struct nearest_away_from_zero : base {
						...
						template<class Float>
						[[nodiscard]] static inline constexpr interval_type::left_closed_right_open
						interval_type_normal( ieee754_bits<Float> ) noexcept {
							return { };
						}
						template<class Float>
					...
					struct nearest_to_even_static_boundary : base {
						...
						template<class Float, class Func>
						[[nodiscard]] static inline constexpr auto delegate( ieee754_bits<Float> br,
						                                                     Func &&f ) noexcept {
							if( br.u % 2 == 0 ) {
				...
				namespace cache {
					...
					struct normal : base {
						...
						template<ieee754_format format>
						[[nodiscard]] static inline constexpr typename cache_holder<format>::cache_entry_type
						get_cache( int k ) noexcept {
							assert( k >= cache_holder<format>::min_k and k <= cache_holder<format>::max_k );
					...
					struct compressed : base {
						...
						get_cache( int k ) noexcept {
							...
							if constexpr( format == ieee754_format::binary64 ) {
								// Compute base index
								auto cache_index =
								  ( k - cache_holder<format>::min_k ) / compressed_cache_detail::compression_ratio;
				...
				namespace input_validation {
					...
					struct do_nothing : base {
						...
						template<class Float>
						static inline constexpr void validate_input( ieee754_bits<Float> ) noexcept {}
					};
				} // namespace input_validation
			}   // namespace policy_impl
		}     // namespace detail
		...
		namespace policy {
			...
			namespace rounding_mode {
				...
				inline constexpr auto toward_zero = detail::policy_impl::rounding_mode::toward_zero{ };
		...
		namespace detail {
			...
			struct impl :
			  ...
			  private ieee754_format_info<ieee754_traits<Float>::format> {
				...
				template<class ReturnType,
				         ...
				         class SignPolicy,
				         class TrailingZeroPolicy,
				         class CorrectRoundingPolicy,
				         class CachePolicy>
				...
				compute_nearest( ieee754_bits<Float> const br ) noexcept {
							...
							if( not interval_type.include_right_endpoint( ) ) {
								...
								if( ( r & mask ) == 0 ) {
									...
									if( div::check_divisibility_and_divide_by_pow5<kappa>( r ) and
									    is_product_integer<integer_check_case_id::fc_pm_half>( two_fr,
									                                                           exponent,
									                                                           minus_k ) ) {
								...
								if( div::check_divisibility_and_divide_by_pow5<kappa>( dist ) ) {
									...
									if( compute_mul_parity( two_fc, cache, beta_minus_1 ) != approx_y_parity ) {
										...
										if constexpr( CorrectRoundingPolicy::tag !=
											...
											if( is_product_integer<integer_check_case_id::fc>( two_fc,
											                                                   exponent,
											                                                   minus_k ) ) {
					...
					if( r > deltai ) {
						...
						if( ( not interval_type.include_left_endpoint( ) or
						      not is_product_integer<integer_check_case_id::fc_pm_half>( two_fl,
						                                                                 exponent,
						                                                                 minus_k ) ) and
				...
				template<class TrailingZeroPolicy,
				         class CorrectRoundingPolicy,
				         class CachePolicy,
				         class ReturnType,
				         class IntervalType>
				DAW_ATTRIB_INLINE JKJ_SAFEBUFFERS static constexpr void
				shorter_interval_case( ReturnType &ret_value,
				                       int const exponent,
				...
				compute_left_closed_directed( ieee754_bits<Float> const br ) noexcept {
					...
					if( not is_product_integer<integer_check_case_id::fc>( significand,
					                                                       exponent + 1,
					                                                       minus_k ) ) {
						++xi;
					}
					...
					if( r > deltai ) {
						...
						if( compute_mul_parity( significand + 1, cache, beta ) or
						    is_product_integer<integer_check_case_id::fc>( significand + 1,
						                                                   exponent + 1,
						                                                   minus_k ) ) {
							return small_divisor_case_label( );
						}
				...
				compute_right_closed_directed( ieee754_bits<Float> const br ) noexcept {
					...
					// Compute k and beta
					int const minus_k =
					  log::floor_log10_pow2( exponent - ( closer_boundary ? 1 : 0 ) ) - kappa;
					auto const cache = CachePolicy::template get_cache<format>( -minus_k );
					int const beta = exponent + log::floor_log2_pow10( -minus_k ) + 1;
					...
					// 10^kappa <= deltai < 10^(kappa + 1)
					auto const deltai =
					  closer_boundary ? compute_delta( cache, beta - 2 ) : compute_delta( cache, beta - 1 );
					...
					if( r > deltai ) {
						...
						// Compare the fractional parts
						if( closer_boundary ) {
							if( not compute_mul_parity( ( significand * 2 ) - 1, cache, beta - 1 ) ) {
								return small_divisor_case_label( );
							}
						} else {
				...
				remove_trailing_zeros( carrier_uint &n ) noexcept {
					constexpr auto max_power = [] {
						auto max_possible_significand = ( std::numeric_limits<carrier_uint>::max )( ) /
					...
					auto t =
					  static_cast<int>( daw::cxmath::count_trailing_zeros( n ) ); // bits::countr_zero( n );
					if( t > max_power ) {
						t = max_power;
					}
					...
					if constexpr( format == ieee754_format::binary32 ) {
						...
						if( s < t and n * divtable.mod_inv[1] <= divtable.max_quotients[1] ) {
						...
						auto remainder =
						  static_cast<std::uint32_t>( static_cast<unsigned>( n ) - 1'0000'0000 * quotient );
				...
				[[nodiscard]] static inline std::uint32_t constexpr compute_delta(
				...
				compute_mul_parity( carrier_uint two_f,
				                    cache_entry_type const &cache,
				                    int beta_minus_1 ) noexcept {
				...
				compute_round_up_for_shorter_interval_case( cache_entry_type const &cache,
				                                            int beta_minus_1 ) noexcept {
					if constexpr( format == ieee754_format::binary32 ) {
						return ( carrier_uint( cache >> ( cache_bits - significand_bits - 2 - beta_minus_1 ) ) +
						         1 ) /
						       2;
					} else {
				...
				is_right_endpoint_integer_shorter_interval( int exponent ) noexcept {
					return exponent >= case_shorter_interval_right_endpoint_lower_threshold and
				...
				is_left_endpoint_integer_shorter_interval( int exponent ) noexcept {
					return exponent >= case_shorter_interval_left_endpoint_lower_threshold and
				...
				[[nodiscard]] static inline constexpr bool
				is_product_integer( carrier_uint two_f, int exponent, int minus_k ) noexcept {
					// Case I: f = fc +- 1/2
					if constexpr( case_id == integer_check_case_id::fc_pm_half ) {
						...
						// For k < 0
							...
							return div::divisible_by_power_of_5<max_power_of_factor_of_5 + 1>(
							  two_f,
							  static_cast<unsigned>( minus_k ) );
						}
					}
					// Case II: f = fc + 1
					// Case III: f = fc
						...
						// Exponent for 2 is negative
							...
							return div::divisible_by_power_of_2(
							  two_f,
							  static_cast<unsigned>( minus_k - exponent + 1 ) );
						}
					}
				}
			};
			...
			namespace policy_impl {
				...
				struct base_default_pair {
					...
					[[nodiscard]] static constexpr FoundPolicyInfo get_policy_impl( FoundPolicyInfo ) {
						return { };
					}
					template<class FoundPolicyInfo, class FirstPolicy, class... RemainingPolicies>
					[[nodiscard]] static constexpr auto
					get_policy_impl( FoundPolicyInfo, FirstPolicy, RemainingPolicies... remainings ) {
					...
					[[nodiscard]] static constexpr auto get_policy( Policies... policies ) {
						return get_policy_impl(
				...
				[[nodiscard]] constexpr bool check_policy_validity( Policy, base_default_pair_list<> ) {
					return false;
				}
				...
				[[nodiscard]] constexpr bool check_policy_validity(
				  ...
				  base_default_pair_list<FirstBaseDefaultPair, RemainingBaseDefaultPairs...> ) {
					return std::is_base_of_v<typename FirstBaseDefaultPair::base, Policy> or
					       check_policy_validity( Policy{ },
					                              base_default_pair_list<RemainingBaseDefaultPairs...>{ } );
				}

				template<class BaseDefaultPairList>
				[[nodiscard]] constexpr bool check_policy_list_validity( BaseDefaultPairList ) {
					return true;
				}
				...
				check_policy_list_validity( BaseDefaultPairList,
					...
					return check_policy_validity( FirstPolicy{ }, BaseDefaultPairList{ } ) and
				...
				template<class FirstBaseDefaultPair,
				         ...
				         bool repeated,
				         class... FoundPolicyPairs,
				         class... Policies>
				[[nodiscard]] constexpr auto make_policy_holder_impl(
				  base_default_pair_list<FirstBaseDefaultPair, RemainingBaseDefaultPairs...>,
				  found_policy_pair_list<repeated, FoundPolicyPairs...>,
				  Policies... policies ) {
				...
				[[nodiscard]] constexpr auto convert_to_policy_holder( found_policy_pair_list<repeated>,
				                                                       RawPolicies... ) {
					return policy_holder<RawPolicies...>{ };
				}

				template<bool repeated,
				         class FirstFoundPolicyPair,
				         class... RemainingFoundPolicyPairs,
				         class... RawPolicies>
				[[nodiscard]] constexpr auto convert_to_policy_holder(
				...
				[[nodiscard]] constexpr auto make_policy_holder( BaseDefaultPairList,
				                                                 Policies... policies ) {
					...
					using policy_pair_list =
					  decltype( make_policy_holder_impl( BaseDefaultPairList{ },
					                                     found_policy_pair_list<false>{ },
					                                     policies... ) );

					static_assert( not policy_pair_list::repeated,
		...
		[[nodiscard]] JKJ_SAFEBUFFERS DAW_ATTRIB_INLINE constexpr auto to_decimal(
		  Float x,
		  Policies... policies ) {
			...
			using return_type =
			  fp_t<Float, policy_holder::return_has_sign, policy_holder::report_trailing_zeros>;
			...
			return policy_holder::delegate( br, [br]( auto interval_type_provider ) {
				...
				} else if constexpr( tag_tmp == rounding_mode::tag_t::left_closed_directed ) {
