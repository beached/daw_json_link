...
namespace daw::json {
	DAW_JSON_INLINE_NS namespace DAW_JSON_VER {
		namespace json_details {
			...
			inline constexpr int max_dbl_exp = std::numeric_limits<double>::max_exponent10;

			template<typename Result>
			inline constexpr int max_exp = std::is_same_v<Result, float>
			                                 ? ( std::min )( max_dbl_exp,
			...
			power10( constexpr_exec_tag, Result result, Unsigned p ) {
				...
				constexpr auto max_v = static_cast<Result>( dpow10_tbl[max_exp<Result>] );

				if( DAW_UNLIKELY( p > max_exp<Result> ) ) {
					if constexpr( std::is_floating_point_v<Result> ) {
						if( DAW_UNLIKELY( p > daw::numeric_limits<Result>::max_exponent ) ) {
					...
					return static_cast<Result>( result ) *
					       ( exp2 * static_cast<Result>( dpow10_tbl[static_cast<std::size_t>( p )] ) );
				} else if( DAW_UNLIKELY( p < -max_exp<Result> ) ) {
					if constexpr( std::is_floating_point_v<Result> ) {
						if( DAW_UNLIKELY( p < daw::numeric_limits<Result>::min_exponent ) ) {
							return Result{ 0 };
						}
					}
					Result exp2 = max_v;
			...
			power10( runtime_exec_tag, Result result, Unsigned p ) {
				if constexpr( std::disjunction<std::is_same<Result, double>,
					...
					return power10( constexpr_exec_tag{ }, result, static_cast<std::int32_t>( p ) );
				} else {
