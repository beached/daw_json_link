...
namespace daw::json {
	DAW_JSON_INLINE_NS namespace DAW_JSON_VER {
		namespace json_details {
			DAW_ATTRIB_INLINE constexpr bool is_escaped( daw::not_null<char const *> ptr,
			...
			DAW_ATTRIB_INLINE __m128i set_reverse( char c0,
			                                       ...
			                                       char c14 = 0,
			                                       char c15 = 0 ) {
				return _mm_set_epi8( c15, c14, c13, c12, c11, c10, c9, c8, c7, c6, c5, c4, c3, c2, c1, c0 );
			}

			DAW_ATTRIB_INLINE __m128i uload16_char_data( sse42_exec_tag,
			                                             daw::not_null<char const *> ptr ) {
				return _mm_loadu_si128(
				  reinterpret_cast<__m128i const *>( static_cast<char const *>( ptr ) ) );
			}
			...
			DAW_ATTRIB_INLINE daw::not_null<CharT *>
			mem_move_to_next_of( sse42_exec_tag tag,
			                     daw::not_null<CharT *> first,
			...
			template<bool is_unchecked_input, char... keys, typename CharT>
			DAW_ATTRIB_INLINE daw::not_null<CharT *>
			mem_move_to_next_not_of( sse42_exec_tag tag,
			                         daw::not_null<CharT *> first,
			                         daw::not_null<CharT *> last ) {
				using keys_len = daw::constant<static_cast<int>( sizeof...( keys ) )>;
				using compare_mode = daw::constant<static_cast<int>( _SIDD_SBYTE_OPS | _SIDD_CMP_EQUAL_ANY |
				...
				while( last - first >= 16 ) {
					...
					int const result = _mm_cmpestri( a, keys_len::value, b, 16, compare_mode::value );
					first += result;
					if( result < 16 ) {
						return first;
					}
				...
				__m128i b{ };
				auto const max_pos = last - first;
				int const result = _mm_cmpestri( a, keys_len::vlaue, b, 16, compare_mode::value );
				if( result < max_pos ) {
					return first + result;
				}
				return last;
			...
			DAW_ATTRIB_INLINE bool add_overflow( U32 value1, U32 value2, U32 &result ) {
				static_assert( sizeof( U32 ) <= sizeof( unsigned long long ) );
				static_assert( sizeof( U32 ) == 4 );
#if( defined( __GNUC__ ) and __GNUC__ >= 8 ) or defined( __clang__ ) or                            \
  ( DAW_HAS_BUILTIN( __builtin_uadd_overflow ) and DAW_HAS_BUILTIN( __builtin_uaddl_overflow ) and \
    DAW_HAS_BUILTIN( __builtin_uaddll_overflow ) )
				if constexpr( sizeof( unsigned ) == sizeof( U32 ) ) {
					...
					return __builtin_uaddl_overflow( static_cast<unsigned long>( value1 ),
					                                 static_cast<unsigned long>( value2 ),
					                                 reinterpret_cast<unsigned long *>( &result ) );
				} else {
					return __builtin_uaddll_overflow( static_cast<unsigned long long>( value1 ),
			...
			find_escaped_branchless( constexpr_exec_tag, UInt32 &prev_escaped, UInt32 backslashes ) {
				backslashes &= ~prev_escaped;
				UInt32 follow_escape = ( backslashes << 1 ) | prev_escaped;
				using even_bits = daw::constant<0x5555'5555_u32>;

				UInt32 const odd_seq_start = backslashes & ( ~even_bits::value ) & ( ~follow_escape );
			...
			DAW_ATTRIB_INLINE UInt32 prefix_xor( sse42_exec_tag, UInt32 bitmask ) {
				...
				__m128i const result =
				  _mm_clmulepi64_si128( _mm_set_epi32( 0, 0, 0, static_cast<std::int32_t>( bitmask ) ),
				                        all_ones,
				                        0 );
			...
			mem_skip_until_end_of_string( simd_exec_tag tag,
				...
				while( last - first >= 16 ) {
					...
					UInt32 const escaped = find_escaped_branchless( tag, prev_escapes, backslashes );
				...
				if constexpr( is_unchecked_input ) {
					...
					while( DAW_LIKELY( first < last ) and *first != '"' ) {
						while( DAW_LIKELY( first < last ) and not key_table<'"', '\\'>[*first] ) {
							++first;
						}
						if( first >= last ) {
							return last;
				...
				return ( is_unchecked_input or DAW_LIKELY( first < last ) ) ? first : last;
			}

			template<bool is_unchecked_input, typename CharT>
			inline daw::not_null<CharT *> mem_skip_until_end_of_string( simd_exec_tag tag,
			                                                            daw::not_null<CharT *> first,
			                                                            daw::not_null<CharT *> const last,
			                                                            std::ptrdiff_t &first_escape ) {
				daw::not_null<CharT *> const first_first = first;
				UInt32 prev_escapes = 0_u32;
				while( last - first >= 16 ) {
					auto const val0 = uload16_char_data( tag, first );
					UInt32 const backslashes = mem_find_eq<'\\'>( tag, val0 );
...
#endif
			template<bool is_unchecked_input, char... keys, typename CharT>
			DAW_ATTRIB_INLINE daw::not_null<CharT *> mem_move_to_next_of( runtime_exec_tag,
			                                                              daw::not_null<CharT *> first,
			                                                              daw::not_null<CharT *> last ) {
				if constexpr( sizeof...( keys ) == 1 ) {
					char const key[]{ keys... };
					auto *ptr = reinterpret_cast<daw::not_null<CharT *>>(
			...
			template<bool is_unchecked_input,
			         typename ExecTag,
			         typename CharT,
			         std::enable_if_t<std::is_base_of<runtime_exec_tag, ExecTag>::value, std::nullptr_t> =
			           nullptr>
			DAW_ATTRIB_INLINE daw::not_null<CharT *>
			mem_skip_string( ExecTag const &tag,
			                 daw::not_null<CharT *> first,
			                 daw::not_null<CharT *> const last ) {
			...
			mem_skip_until_end_of_string( ExecTag const &tag,
				...
				first = mem_move_to_next_of<is_unchecked_input, '\\', '"'>( tag, first, last );
				while( is_unchecked_input or first < last ) {
					switch( *first ) {
					case '"':
						return first;
