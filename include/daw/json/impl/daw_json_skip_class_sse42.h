	...
	inline namespace DAW_JSON_VER {
		namespace json_details {
			namespace skip_bracketed_item_sse42_details {
				...
				[[nodiscard]] DAW_ATTRIB_INLINE __m128i
				load( char const *first, std::size_t count ) noexcept {
					if( count == block_size ) {
						return _mm_loadu_si128(
						  reinterpret_cast<__m128i const *>( first ) );
					}
			...
			/// Skip the JSON class or array at parse_state.first using SSE4.2.
			...
			skip_bracketed_item_sse42( ParseState &parse_state ) {
				...
				using namespace skip_bracketed_item_sse42_details;
				...
				while( ptr_first < ptr_last ) {
					...
					in_string = ( ( in_string_bits >> ( lane_count - 1U ) ) & 1U ) != 0;
