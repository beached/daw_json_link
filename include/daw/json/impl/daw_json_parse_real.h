...
namespace daw::json {
	DAW_JSON_INLINE_NS namespace DAW_JSON_VER {
		namespace json_details {
			...
			constexpr unsigned round_to_msd( daw::not_null<char const *> first,
				...
				do {
					--last;
					auto dig = static_cast<unsigned>( static_cast<unsigned char>( *last ) - '0' );
					if( dig >= 10U ) {
						// Account for '.'
						continue;
					}
				...
				auto const result = ( ( carry + ( static_cast<unsigned char>( *first ) - '0' ) ) + 5 ) / 10;
				return result;
			}
			...
			DAW_ATTRIB_FLATINLINE inline constexpr daw::not_null<CharT *>
			parse_digits_while_number( daw::not_null<CharT *> first,
				...
				Unsigned value = v;
				if constexpr( skip_end_check ) {
					for( auto dig = parse_digit( *first ); dig < 10U; ++first, dig = parse_digit( *first ) ) {
						value *= 10U;
						value += dig;
					}
				} else {
					unsigned dig = 0;
					for( ; DAW_LIKELY( first < last ) and ( dig = parse_digit( *first ) ) < 10U; ++first ) {
						value *= 10U;
						value += dig;
					}
				}
			...
			parse_real( ParseState &parse_state ) {
				...
				auto whole_first = daw::not_null( daw::never_null, parse_state.first );
				CharT *whole_last =
				  parse_state.class_first ? parse_state.class_first : parse_state.class_last;
				CharT *fract_first = parse_state.class_first ? parse_state.class_first + 1 : nullptr;
				CharT *fract_last = parse_state.class_last;
				CharT *exp_first = parse_state.class_last ? parse_state.class_last + 1 : nullptr;
				auto const exp_last = daw::not_null( daw::never_null, parse_state.last );
				...
				bool use_strtod = [&] {
					if constexpr( std::is_floating_point_v<Result> and ParseState::precise_ieee754 ) {
						return DAW_UNLIKELY(
						  ( ( whole_last - whole_first ) + ( fract_first ? fract_last - fract_first : 0 ) ) >
				...
				using unsigned_t = std::
				  conditional_t<max_storage_digits::value >= max_exponent::value, std::uint64_t, Result>;

				using signed_t = typename std::conditional_t<std::is_floating_point_v<unsigned_t>,
				...
				std::intmax_t fract_exponent_available = fract_first ? fract_last - fract_first : 0;
				signed_t exponent = 0;
				...
				if( whole_exponent_available > max_exponent::value ) {
					...
					whole_exponent_available = max_exponent::value - whole_exponent_available;
					if constexpr( ParseState::precise_ieee754 ) {
						use_strtod |= DAW_UNLIKELY( fract_exponent_available > whole_exponent_available );
					}
				...
				parse_digits_until_last<( ParseState::is_zero_terminated_string or
				                          ParseState::is_unchecked_input )>( whole_first,
				                                                             whole_last,
				                                                             significant_digits );
				if( fract_first ) {
					parse_digits_until_last<( ParseState::is_zero_terminated_string or
					                          ParseState::is_unchecked_input )>( fract_first,
					                                                             fract_last,
					                                                             significant_digits );
				}
				...
				if( exp_first and ( exp_last - exp_first ) > 0 ) {
					signed_t const exp_sign = [&] {
						...
						case '-':
							++exp_first;
							daw_json_assert_weak( exp_first < exp_last, ErrorReason::InvalidNumber );
							return -1;
						case '+':
							daw_json_assert_weak( exp_first < exp_last, ErrorReason::InvalidNumber );
							++exp_first;
				...
				if constexpr( std::is_floating_point_v<Result> and ParseState::precise_ieee754 ) {
					...
					use_strtod |= DAW_UNLIKELY( exponent < -22 );
					use_strtod |= DAW_UNLIKELY( significant_digits > 9007199254740992ULL );
					if( DAW_UNLIKELY( use_strtod ) ) {
						return json_details::parse_with_strtod<Result>( parse_state.first, parse_state.last );
					}
				}
			...
			parse_real( ParseState &parse_state ) {
				...
				auto const orig_first = daw::not_null( daw::never_null, parse_state.first );
				auto const orig_last = daw::not_null( daw::never_null, parse_state.last );
				...
				auto const sign =
				  static_cast<Result>( parse_policy_details::validate_signed_first( parse_state ) );

				using max_storage_digits =
				  daw::constant<static_cast<std::int64_t>( daw::numeric_limits<std::uint64_t>::digits10 )>;
				using max_exponent =
				  daw::constant<static_cast<std::int64_t>( daw::numeric_limits<Result>::max_digits10 + 1 )>;
				using unsigned_t = std::
				  conditional_t<max_storage_digits::value >= max_exponent::value, std::uint64_t, Result>;
				using signed_t = std::
				  conditional_t<max_storage_digits::value >= max_exponent::value, std::int64_t, Result>;

				auto first = daw::not_null( daw::never_null, parse_state.first );
				auto const whole_last = daw::not_null(
				  parse_state.first + ( std::min )( parse_state.last - parse_state.first,
				                                    static_cast<std::ptrdiff_t>( max_exponent::value ) ) );

				unsigned_t significant_digits = 0;
				daw_json_assert_weak( not( whole_last - first > 1 and *first == '0' and
				...
				auto last_char =
				  parse_digits_while_number<( ParseState::is_zero_terminated_string or
				                              ParseState::is_unchecked_input )>( first,
				                                                                 whole_last,
				                                                                 significant_digits );

				std::ptrdiff_t sig_digit_count = last_char - parse_state.first;
				bool use_strtod = std::is_floating_point_v<Result> and ParseState::precise_ieee754 and
				...
				signed_t exponent_p1 = [&] {
					if( DAW_UNLIKELY( last_char >= whole_last ) ) {
						...
						auto ptr = skip_digits<( ParseState::is_zero_terminated_string or
				...
				first = last_char;
				if( ( ParseState::is_zero_terminated_string or ParseState::is_unchecked_input or
				    ...
				    *first == '.' ) {
					...
					if( exponent_p1 != 0 ) {
						...
						auto const cur_digit_count = static_cast<std::ptrdiff_t>( first - parse_state.first );
						auto const sig_digits_avail =
						  static_cast<std::ptrdiff_t>( max_exponent::value - cur_digit_count );
						auto const fract_last_diff = ( std::min )( max_digits_left, sig_digits_avail );
						auto fract_last = first + fract_last_diff;
						...
						daw_json_assert_weak( DAW_LIKELY( first < last_char ) or
						...
						first = last_char;
						if( daw::nsc_and( ( first >= fract_last ), ( first < parse_state.last ) ) ) {
							auto new_first = skip_digits<( ParseState::is_zero_terminated_string or
							                               ParseState::is_unchecked_input )>(
							  first,
							  daw::not_null( daw::never_null, parse_state.last ) );
							if constexpr( std::is_floating_point_v<Result> and ParseState::precise_ieee754 ) {
					...
					if( ( ParseState::is_unchecked_input or first < parse_state.last ) and
						...
						signed_t const exp_sign = [&] {
							daw_json_assert_weak(
							  ( ParseState::is_zero_terminated_string or first < parse_state.last ),
							...
							case '-':
								++first;
								daw_json_assert_weak( first < parse_state.last and parse_digit( *first ) < 10U,
								                      ErrorReason::InvalidNumber );
								return signed_t{ -1 };
							default:
						...
						last_char = parse_digits_while_number<( ParseState::is_zero_terminated_string or
						                                        ParseState::is_unchecked_input )>(
						  first,
						  daw::not_null( daw::never_null, parse_state.last ),
						  exp_tmp );
				...
				signed_t const exponent = [&] {
					if constexpr( ParseState::is_unchecked_input or not std::is_floating_point_v<Result> ) {
						return exponent_p1 + exponent_p2;
					} else {
						if( bool matching_signs = ( exponent_p1 < 0 ) == ( exponent_p2 < 0 );
						    not matching_signs ) {
						...
						if( s < 0 ) {
							if( DAW_UNLIKELY( ( daw::numeric_limits<signed_t>::min( ) - exponent_p1 ) >
							                  exponent_p2 ) ) {
						...
						auto r =
						  static_cast<unsigned_t>( exponent_p1 ) + static_cast<unsigned_t>( exponent_p2 );
						if( DAW_UNLIKELY( r >
						                  static_cast<unsigned_t>( daw::numeric_limits<signed_t>::max( ) ) ) ) {
				...
				if constexpr( std::is_floating_point_v<Result> and ParseState::precise_ieee754 ) {
					...
					use_strtod |= DAW_UNLIKELY( significant_digits > 9007199254740992ULL );
					if( DAW_UNLIKELY( use_strtod ) ) {
						return json_details::parse_with_strtod<Result>( orig_first, orig_last );
					}
				}
				return sign * power10<Result>( ParseState::exec_tag,
				                               static_cast<Result>( significant_digits ),
