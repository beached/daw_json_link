...
namespace daw::json {
	DAW_JSON_INLINE_NS namespace DAW_JSON_VER {
		...
		namespace json_details {
			...
			inline constexpr auto json_class_constructor = json_class_constructor_t<T, Default>{ };
			...
			construct_value( template_params<Value, Constructor>,
				...
				if constexpr( ParseState::has_allocator ) {
					...
					if constexpr( std::is_invocable<Constructor, Args..., alloc_t>::value ) {
						return Constructor{ }( DAW_FWD2( Args, args )..., DAW_MOVE( alloc ) );
			...
			struct construct_value_tp_invoke_t {
				...
				operator( )( fwd_pack<TArgs...> &&tp, std::index_sequence<Is...> ) const {
					return Constructor{ }( get<Is>( DAW_MOVE( tp ) )... );
				}

				template<typename... TArgs, typename Allocator, std::size_t... Is>
				DAW_ATTRIB_FLATINLINE inline constexpr decltype( auto )
				operator( )( fwd_pack<TArgs...> &&tp, Allocator &alloc, std::index_sequence<Is...> ) const {
					return Constructor{ }( get<Is>( DAW_MOVE( tp ) )..., DAW_FWD2( Allocator, alloc ) );
				}

				template<typename Alloc, typename... TArgs, std::size_t... Is>
				DAW_ATTRIB_FLATINLINE inline constexpr decltype( auto )
				operator( )( std::allocator_arg_t,
				             Alloc &&alloc,
				             fwd_pack<TArgs...> &&tp,
				             std::index_sequence<Is...> ) const {

					return Constructor{ }( std::allocator_arg,
					                       DAW_FWD2( Alloc, alloc ),
					                       get<Is>( DAW_MOVE( tp ) )... );
				}
			};

			template<typename Constructor>
			inline constexpr auto construct_value_tp_invoke = construct_value_tp_invoke_t<Constructor>{ };

			template<typename Value, typename Constructor, typename ParseState, typename... Args>
			[[maybe_unused]] DAW_ATTRIB_FLATINLINE static inline constexpr auto
			construct_value_tp( ParseState &parse_state, fwd_pack<Args...> &&tp_args ) {
				if constexpr( ParseState::has_allocator ) {
					using alloc_t = typename ParseState::template allocator_type_as<Value>;
					auto alloc = parse_state.get_allocator_for( template_arg<Value> );
					if constexpr( std::is_invocable<Constructor, Args..., alloc_t>::value ) {
						return construct_value_tp_invoke<Constructor>( DAW_MOVE( tp_args ),
						                                               DAW_MOVE( alloc ),
					                                                ...
					                                                alloc_t,
					                                                Args...> ) {
						return construct_value_tp_invoke<Constructor>( std::allocator_arg,
						                                               DAW_MOVE( alloc ),
						                                               DAW_MOVE( tp_args ),
						                                               std::index_sequence_for<Args...>{ } );
					} else {
			...
			using has_deallocate_test =
			  decltype( std::declval<Allocator &>( ).deallocate( static_cast<void *>( nullptr ),
			                                                     size_t{ 1 } ) );

			template<typename Allocator>
			inline constexpr bool is_allocator_v =
			  std::conjunction<daw::is_detected<has_allocate_test, Allocator>,
			                   daw::is_detected<has_deallocate_test, Allocator>>::value;

			template<typename T>
			using has_json_data_contract_trait =
			  not_trait<std::is_same<missing_json_data_contract_for<T>, json_data_contract_trait_t<T>>>;
			template<typename T>
			inline constexpr bool has_json_data_contract_trait_v = has_json_data_contract_trait<T>::value;

			template<typename Container, typename Value>
			using detect_push_back =
			  decltype( std::declval<Container &>( ).push_back( std::declval<Value>( ) ) );

			template<typename Container, typename Value>
			using detect_insert_end =
			  decltype( std::declval<Container &>( ).insert( std::end( std::declval<Container &>( ) ),
			                                                 std::declval<Value>( ) ) );
			...
			using json_result = typename std::conditional_t<is_no_name_v<JsonMember>,
			...
			struct basic_appender {
				...
				DAW_ATTRIB_FLATINLINE inline constexpr void operator( )( Value &&value ) const {
					if constexpr( has_push_back_v<Container, daw::remove_cvref_t<Value>> ) {
						m_container->push_back( DAW_FWD2( Value, value ) );
					} else if constexpr( has_insert_end_v<Container, daw::remove_cvref_t<Value>> ) {
						m_container->insert( std::end( *m_container ), DAW_FWD2( Value, value ) );
					} else {
						static_assert( has_push_back_v<Container, daw::remove_cvref_t<Value>> or
				...
				template<
				  typename Value,
				  std::enable_if_t<not std::is_same<basic_appender, daw::remove_cvref_t<Value>>::value,
			...
			template<typename JsonType>
			static inline constexpr bool is_json_nullable_v = is_json_nullable<JsonType>::value;
			/***
			...
			inline constexpr json_options_t
			  class_opts_set = set_bits( class_opts, CurrentOptions, option, options... );

			template<json_options_t CurrentOptions, auto option, auto... options>
			inline constexpr json_options_t
			  number_opts_set = set_bits( number_opts, CurrentOptions, option, options... );

			template<json_options_t CurrentOptions, auto option, auto... options>
			inline constexpr json_options_t
			  bool_opts_set = set_bits( bool_opts, CurrentOptions, option, options... );

			template<json_options_t CurrentOptions, auto option, auto... options>
			inline constexpr json_options_t
			  string_opts_set = set_bits( string_opts, CurrentOptions, option, options... );

			template<json_options_t CurrentOptions, auto option, auto... options>
			inline constexpr json_options_t
			  string_raw_opts_set = set_bits( string_raw_opts, CurrentOptions, option, options... );

			template<json_options_t CurrentOptions, auto option, auto... options>
			inline constexpr json_options_t
			  json_custom_opts_set = set_bits( json_custom_opts, CurrentOptions, option, options... );

			template<json_options_t CurrentOptions, auto option, auto... options>
			inline constexpr json_options_t
			  tuple_opts_set = set_bits( tuple_opts, CurrentOptions, option, options... );
		} // namespace json_details
		...
		namespace json_base {
			...
			using json_class_null =
			  json_class<T, Constructor, json_details::class_opts_set<Options, JsonNullDefault>>;
			...
			using json_bool_null =
			  json_bool<T, json_details::bool_opts_set<Options, JsonNullDefault>, Constructor>;
			...
			template<typename Tuple,
			         ...
			         json_details::json_options_t Options = class_opts_def,
			         typename JsonTupleTypesList = json_deduce_type>
			using json_tuple_null = json_tuple<Tuple,
			                                   Constructor,
		...
		namespace json_details {
			...
			template<typename T>
			inline constexpr JsonParseTypes number_parse_type_v = number_parse_type_test<T>( );
			...
			template<typename JsonType>
			struct json_deduced_type_map<JsonType,
			                             std::enable_if_t<json_details::is_a_json_type_v<JsonType>>> {
				static constexpr bool is_null = false;
				static constexpr JsonParseTypes parse_type = JsonParseTypes::Unknown;
			...
			struct json_deduced_type_map<
			  T,
			  std::enable_if_t<json_details::has_json_data_contract_trait_v<T>>> {
				static constexpr bool is_null = false;
			...
			struct json_deduced_type_map<std::string> {
				static constexpr bool is_null = false;
				static constexpr JsonParseTypes parse_type = JsonParseTypes::StringEscaped;
			...
			struct json_deduced_type_map<Integer,
				...
				static constexpr JsonParseTypes parse_type =
				  daw::is_signed_v<Integer> ? JsonParseTypes::Signed : JsonParseTypes::Unsigned;

				static constexpr bool type_map_found = true;
			};

			template<typename Enum>
			struct json_deduced_type_map<Enum,
			                             std::enable_if_t<std::conjunction_v<
			                               not_trait<json_details::has_json_data_contract_trait<Enum>>,
			                               std::is_enum<Enum>>>> {
				static constexpr bool is_null = false;
				static constexpr JsonParseTypes parse_type = daw::is_signed_v<std::underlying_type<Enum>>
			...
			struct json_deduced_type_map<
			  FloatingPoint,
			  std::enable_if_t<
			    std::conjunction_v<not_trait<json_details::has_json_data_contract_trait<FloatingPoint>>,
			...
			template<typename Tuple>
			struct json_deduced_type_map<Tuple,
			                             std::enable_if_t<std::conjunction_v<
			...
			template<typename T>
			using is_container =
			  std::conjunction<daw::is_detected<container_detect::container_value_type, T>,
			...
			using is_string =
			  std::conjunction<is_container<String>, container_detect::is_value_type_char<String>>;
			...
			using is_associative_container = std::conjunction<
			  ...
			  daw::is_detected<container_detect::container_key_type, AssociativeContainer>,
			  daw::is_detected<container_detect::container_mapped_type, AssociativeContainer>>;
			...
			template<typename AssociativeContainer>
			struct json_deduced_type_map<AssociativeContainer,
			                             std::enable_if_t<std::conjunction_v<
			...
			struct json_deduced_type_map<
			  Container,
			  std::enable_if_t<std::conjunction_v<not_trait<has_json_data_contract_trait<Container>>,
			...
			template<typename T>
			struct json_deduced_type_map<std::optional<T>,
			                             std::enable_if_t<std::conjunction_v<
			...
			template<typename T>
			inline constexpr bool has_deduced_type_mapping_v = json_deduced_type_map<T>::type_map_found;
			...
			template<auto Value, auto... Values>
			inline constexpr bool equal_to_one_of_v = equal_to_one_of<Value, Values...>::value;
			...
			inline constexpr auto json_link_quick_map( ) {
				if constexpr( is_a_json_type_v<T> ) {
					...
					if constexpr( parse_type::value == JsonParseTypes::Unknown ) {
					...
					} else if constexpr( parse_type::value == JsonParseTypes::StringRaw ) {
						if constexpr( is_null::value ) {
							return json_link_quick_map_type<json_base::json_string_raw_null<T>>{ };
						} else {
							return json_link_quick_map_type<json_base::json_string_raw<T>>{ };
						}
					} else if constexpr( parse_type::value == JsonParseTypes::StringEscaped ) {
						if constexpr( is_null::value ) {
						...
						if constexpr( is_null::value ) {
							...
							return json_link_quick_map_type<json_base::json_bool<T>>{ };
					                                       ...
					                                       JsonParseTypes::Real> ) {
						if constexpr( is_null::value ) {
							return json_link_quick_map_type<json_base::json_number_null<T>>{ };
						} else {
						...
						if constexpr( is_null::value ) {
							...
							return json_link_quick_map_type<json_base::json_key_value_null<T, v_t, k_t>>{ };
						} else {
							using k_t = typename mapped_type_t::key;
							using v_t = typename mapped_type_t::value;
							return json_link_quick_map_type<json_base::json_key_value<T, v_t, k_t>>{ };
						}
					} else if constexpr( parse_type::value == JsonParseTypes::Array ) {
						if constexpr( is_null::value ) {
							using b_t = typename mapped_type_t::base_type;
							using v_t = typename b_t::value;
							return json_link_quick_map_type<json_base::json_array_null<v_t, T>>{ };
						} else {
							using v_t = typename mapped_type_t::value;
			...
			template<typename T>
			inline constexpr bool has_json_link_quick_map_v = has_json_link_quick_map<T>::value;
			...
			template<typename T,
			         /* typename Ordered, */ bool Contract,
			         bool JsonType,
			         bool QuickMap,
			         bool Container>
			struct json_type_deducer<T, true, Contract, JsonType, QuickMap, Container> {
			...
			struct as_json_class {
				using type = json_base::json_class<T, json_class_constructor_t<T, default_constructor<T>>>;
			};

			template<typename T,
			         /*typename Contract,*/ bool JsonType,
			         bool QuickMap,
			         bool Container>
			struct json_type_deducer<T, false, true, JsonType, QuickMap, Container> {
				...
				static_assert( not std::is_same_v<daw::remove_cvref_t<type>, void>, "Detection failure" );
				static_assert( not std::is_same_v<daw::remove_cvref_t<type>, daw::nonesuch>,
				               "Detection failure" );
			};

			template<typename T,
			         /*bool Contract, bool JsonType,*/ bool QuickMap,
			         bool Container>
			...
			template<typename T /*bool Contract, bool JsonType, bool QuickMap*/, bool Container>
			...
			template<typename T>
			using json_deduced_type = typename json_type_deducer<T,
			                                                     is_an_ordered_member_v<T>,
			                                                     has_json_data_contract_trait_v<T>,
			                                                     json_details::is_a_json_type_v<T>,
			...
			template<typename JsonMember>
			using is_never_constexpr =
			  daw::detected_or_t<std::false_type, is_never_constexpr_test, JsonMember>;
			...
			inline constexpr bool use_direct_construction_v =
			  ...
			  ( ( not ParsePolicy::exec_tag_t::can_constexpr or ParsePolicy::always_runtime or
			      is_never_constexpr<JsonMember>::value ) and
			    ( not ParsePolicy::has_allocator and std::is_aggregate_v<json_result<JsonMember>> and
			      is_default_constructor_v<typename JsonMember::constructor_t> ) );

			template<typename T>
			using has_json_deduced_type =
			  daw::not_trait<std::is_same<json_deduced_type<T>, missing_json_data_contract_for<T>>>;

			template<typename T>
			inline constexpr bool has_unnamed_default_type_mapping_v = has_json_deduced_type<T>::value;
			...
			using json_class_parse_result_impl2 =
			  decltype( Constructor{ }( std::declval<json_result<Members> &&>( )... ) );
			...
			using json_class_parse_result_impl =
			  daw::detected_or_t<unable_determine_class_parse_result<Constructor, Members...>,
