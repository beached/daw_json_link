	...
	DAW_JSON_INLINE_NS namespace DAW_JSON_VER {
		namespace fp_utils {
			...
			struct fp_properties<float> {
				...
				static constexpr std::uint32_t bit_width =
				  sizeof( unsigned_t ) * CHAR_BIT;

				static constexpr std::uint32_t mantissa_width = 23;
				static constexpr std::uint32_t exponent_width = 8;
				static constexpr unsigned_t mantissa_mask =
				  ( unsigned_t{ 1U } << mantissa_width ) - 1U;
				static constexpr unsigned_t sign_mask =
				  unsigned_t{ 1U } << ( exponent_width + mantissa_width );
				static constexpr unsigned_t exponent_mask =
			...
			struct fp_properties<double> {
				...
				static constexpr unsigned_t mantissa_mask =
				  ( unsigned_t{ 1U } << mantissa_width ) - 1;
				static constexpr unsigned_t sign_mask =
				  unsigned_t{ 1U } << ( exponent_width + mantissa_width );
				static constexpr unsigned_t exponent_mask =
				  ~( sign_mask | mantissa_mask );
