		...
		namespace fp_utils {
			...
			struct fp_data {
				...
				constexpr void set_unbiased_exponent( unsigned_t expVal ) noexcept {
					expVal =
					  ( expVal << ( fp_prop::mantissa_width ) ) & fp_prop::exponent_mask;
					bits &= ~( fp_prop::exponent_mask );
					bits |= expVal;
				}

				constexpr uint16_t get_unbiased_exponent( ) const noexcept {
					return uint16_t( ( bits & fp_prop::exponent_mask ) >>
					                 ( fp_prop::mantissa_width ) );
				}

				constexpr void set_sign( bool sign_val ) noexcept {
					bits &= ~( fp_prop::sign_mask );
					unsigned_t sign = unsigned_t( sign_val )
					                  << ( fp_prop::bit_width - 1 );
					bits |= sign;
				}

				constexpr bool get_sign( ) const noexcept {
					return ( ( bits & fp_prop::sign_mask ) >>
					         ( fp_prop::bit_width - 1 ) );
				}

				static_assert(
				  sizeof( Float ) == sizeof( unsigned_t ),
				...
				template<
				  typename F,
				  std::enable_if_t<std::is_same_v<Float, F>, std::nullptr_t> = nullptr>
				explicit constexpr fp_data( F float_value ) noexcept
				  : bits( DAW_BIT_CAST( Float, float_value ) ) {}

				template<typename U,
				         std::enable_if_t<std::is_same_v<unsigned_t, U>,
				...
				constexpr bool is_inf( ) const noexcept {
					return get_mantissa( ) == 0 and
					       get_unbiased_exponent( ) == max_exponent;
				}

				constexpr bool is_NaN( ) const noexcept {
					return get_unbiased_exponent( ) == max_exponent and
					       get_mantissa( ) != 0;
				}
				...
				static constexpr fp_data neg_zero( ) noexcept {
					return fp_data( unsigned_t{ 1U }
					                << ( sizeof( unsigned_t ) * CHAR_BIT - 1U ) );
				}
